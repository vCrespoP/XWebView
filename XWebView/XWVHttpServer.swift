...
class XWVHttpServer : NSObject {
    ...
    fileprivate(set) var port: in_port_t = 0
    ...
    fileprivate func listenOnPort(_ port: in_port_t) -> Bool {
        ...
        let data = Data(bytes: UnsafePointer<UInt8>(&sockaddr), count: sizeof(sockaddr_in))
        guard CFSocketSetAddress(socket, data) == CFSocketError.success else {
            log("!Failed to listen on port \(port) \(String(validatingUTF8: strerror(errno))!)")
    ...
    func start(_ port: in_port_t = 0) -> Bool {
        if port == 0 {
    ...
    func handleRequest(_ request: URLRequest) -> HTTPURLResponse {
        ...
        if request.url == nil {
            ...
            if fileURL.path != nil {
                ...
                headers["Content-Length"] = String(attrs[FileAttributeKey.size]!)
...
private func ServerAcceptCallBack(_ socket: CFSocket!, type: CFSocketCallBackType, address: CFData!, data:UnsafeRawPointer, info: UnsafeMutableRawPointer) {
...
private func getMIMETypeByExtension(_ extensionName: String) -> String {
